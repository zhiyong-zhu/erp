...
桌面端定位为现场操作客户端，不承载用户、部门、角色、权限等系统管理页面；系统管理统一由 Web 后台完成。桌面端优先落地创建工单、生产执行、扫码入库、扫码出库、仓库盘点、箱码打印、出入库单据打印等高频生产仓储操作。
...
- 移动端定位为现场操作客户端，不承载用户、部门、角色、权限等系统管理页面；系统管理统一由 Web 后台完成。

#### 4.6.1 移动端页面清单

| 页面 | 导航层级 | 优先级 | 对应角色 | 说明 |
|------|---------|--------|---------|------|
| **移动作业台** | Tab首页 | P0 | 车间/仓库操作员 | 创建工单、生产执行、扫码出入库、盘点、打印入口 |
| **创建工单** | Tab首页入口 | P0 | 生产主管/班组长 | 选择产品、数量、计划时间并创建生产工单 |
| **生产执行** | Tab首页入口 | P0 | 生产工人 | 扫描/选择工单 → 填报良品、不良和工时 |
| **扫码出入库** | Tab首页入口 | P0 | 仓管员 | 摄像头扫码 → 匹配单据 → 确认操作 |
| **仓库盘点** | Tab首页入口 | P0 | 仓管员 | 按仓库/库位扫码录入实盘数量 |
| **箱码打印** | 二级页 | P0 | 车间/仓库操作员 | 按工单和装箱数量生成并打印箱码 |
| **出入库单据打印** | 二级页 | P0 | 仓管员 | 预览并打印入库单、出库单、盘点单 |
